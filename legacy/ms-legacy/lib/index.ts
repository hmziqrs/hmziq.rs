...
export interface WASMModule {
  ...
  precalculate_cubic_bezier_path: (
    p0x: number,
    p0y: number,
    p1x: number,
    p1y: number,
  ...
  batch_interpolate_meteor_positions: (
    life_values: Float32Array,
    max_life_values: Float32Array,
    path_data: Float32Array,
    path_stride: number
  ) => Float32Array
  ...
  loadPromise = (async () => {
    try {
      ...
      if (debugConfig.isEnabled('enableConsoleLogs')) {
        console.log(
          'WASM module loaded successfully with star field, meteor optimizations, and unified particle manager'
        )
      }
    } catch (error) {
...
export const jsFallbacks: WASMModule = {
  ...
  precalculate_bezier_path: (
    ...
    for (let i = 0; i <= segments; i++) {
      ...
      const x = oneMinusT * oneMinusT * start_x + 2 * oneMinusT * t * control_x + t * t * end_x

      const y = oneMinusT * oneMinusT * start_y + 2 * oneMinusT * t * control_y + t * t * end_y
  ...
  precalculate_cubic_bezier_path: (
    ...
    for (let i = 0; i <= segments; i++) {
      ...
      const x =
        oneMinusTCube * p0x + 3 * oneMinusTSq * t * p1x + 3 * oneMinusT * tSq * p2x + tCube * p3x

      const y =
        oneMinusTCube * p0y + 3 * oneMinusTSq * t * p1y + 3 * oneMinusT * tSq * p2y + tCube * p3y
