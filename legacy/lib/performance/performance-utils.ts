...
export function calculateBezierPathUniform(
  ...
  for (let i = 0; i <= segments; i++) {
    ...
    const segmentEndLength =
      segmentIdx + 1 < arcLengths.length ? arcLengths[segmentIdx + 1] : totalLength

    const segmentT =
      segmentEndLength > segmentStartLength
        ? (targetLength - segmentStartLength) / (segmentEndLength - segmentStartLength)
        : 0

    const p1 = tempPoints[segmentIdx]
    const p2 =
      segmentIdx + 1 < tempPoints.length
        ? tempPoints[segmentIdx + 1]
...
export function interpolateBezierPoint(points: BezierPoint[], t: number): BezierPoint {
