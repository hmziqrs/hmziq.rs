            ...
            for x in (0..width as usize).step_by(skip_size) {
                ...
                if index + 3 < pixel_data.len() {
                    ...
                    if alpha > 128 && particle_index < MAX_PARTICLES {
                        ...
                        state.positions_x[particle_index] = js_sys::Math::random() as f32 * canvas_width;
                        state.positions_y[particle_index] = js_sys::Math::random() as f32 * canvas_height;
...
fn update_particle_batch_simd(state: &mut ScatterTextState, base: usize) {
    unsafe {
        ...
        if state.forming {
            ...
            v128_store(state.positions_x[base + 4..].as_mut_ptr() as *mut v128, new_x2);
            v128_store(state.positions_x[base + 8..].as_mut_ptr() as *mut v128, new_x3);
            v128_store(state.positions_x[base + 12..].as_mut_ptr() as *mut v128, new_x4);
            
            v128_store(state.positions_y[base..].as_mut_ptr() as *mut v128, new_y);
            v128_store(state.positions_y[base + 4..].as_mut_ptr() as *mut v128, new_y2);
            v128_store(state.positions_y[base + 8..].as_mut_ptr() as *mut v128, new_y3);
            v128_store(state.positions_y[base + 12..].as_mut_ptr() as *mut v128, new_y4);
            
            // Reset opacity when forming
            let full_opacity = f32x4_splat(1.0);
            v128_store(state.opacity[base..].as_mut_ptr() as *mut v128, full_opacity);
            v128_store(state.opacity[base + 4..].as_mut_ptr() as *mut v128, full_opacity);
            v128_store(state.opacity[base + 8..].as_mut_ptr() as *mut v128, full_opacity);
            v128_store(state.opacity[base + 12..].as_mut_ptr() as *mut v128, full_opacity);
        } else {
            // Scattering
            ...
            v128_store(state.opacity[base + 4..].as_mut_ptr() as *mut v128, new_opacity2);
            v128_store(state.opacity[base + 8..].as_mut_ptr() as *mut v128, new_opacity3);
            v128_store(state.opacity[base + 12..].as_mut_ptr() as *mut v128, new_opacity4);
            ...
            v128_store(state.scatter_vx[base + 4..].as_mut_ptr() as *mut v128, new_vx2);
            v128_store(state.scatter_vx[base + 8..].as_mut_ptr() as *mut v128, new_vx3);
            v128_store(state.scatter_vx[base + 12..].as_mut_ptr() as *mut v128, new_vx4);
            
            v128_store(state.scatter_vy[base..].as_mut_ptr() as *mut v128, new_vy);
            v128_store(state.scatter_vy[base + 4..].as_mut_ptr() as *mut v128, new_vy2);
            v128_store(state.scatter_vy[base + 8..].as_mut_ptr() as *mut v128, new_vy3);
            v128_store(state.scatter_vy[base + 12..].as_mut_ptr() as *mut v128, new_vy4);
        }
    }
}
